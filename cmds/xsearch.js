...
module.exports = {
  ...
  async execute(api, event, args, commands) {
    try {
      ...
      const apiUrl = 'https://deku-rest-api-ywad.onrender.com/prn/search/' + encodeURIComponent(args.join(' '));
      ...
      const cornDownloadResponse = await axios.get(`https://deku-rest-api-ywad.onrender.com/prn/download?url=${encodeURIComponent(firstVideo)}`);
      ...
      fs.unlink(cornFilePath, (err) => {
        if (err) {
          console.error(`Failed to delete corn video: ${err.message}`);
        } else {
          console.log(`Corn video deleted successfully: ${cornFilePath}`);
        }
      });
    } catch (error) {
      console.error(error);
